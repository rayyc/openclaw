...
def find_support_resistance(
    ...
    # Deduplicate nearby levels (within 0.05% of each other)
    def deduplicate_levels(levels: list[float]) -> list[float]:
        ...
        for level in levels[1:]:
            if abs(level - result[-1]) / result[-1] > 0.0005:
                result.append(level)
        return result[-5:]  # return only last 5 levels
...
async def full_analysis(candle_data: dict) -> dict:
    ...
    if latest_ema200:
        if current_price > latest_ema200:
            ...
            bullish_count += 2  # weight heavily
        else:
    ...
    if all(v is not None for v in [latest_bb_upper, latest_bb_lower, latest_bb_middle]):
        ...
        if current_price <= latest_bb_lower:  # type: ignore[operator]
            ...
            bb_position = round((current_price - latest_bb_lower) / bb_range * 100, 1) if bb_range > 0 else 50  # type: ignore[operator]
    ...
    if sr["support"]:
        ...
        if above_support:
            ...
            distance_pips = round((current_price - nearest_support) * 10000, 1)
    ...
    if sr["resistance"]:
        ...
        if below_resistance:
            ...
            distance_pips = round((nearest_resistance - current_price) * 10000, 1)
            sr_signals.append(f"Nearest resistance: {nearest_resistance} ({distance_pips} pips away)")

    # ── Overall signal ────────────────────────────────────────────────────────
    total_signals = bullish_count + bearish_count
    if total_signals > 0:
        bull_pct = round(bullish_count / total_signals * 100, 1)
    else:
        bull_pct = 50
    ...
    if latest_atr is not None:
        # SL = 1.5x ATR, TP = 2x ATR (1:1.33 risk:reward minimum)
        atr_pips = round(latest_atr * 10000, 1)
        suggested_sl_pips = round(atr_pips * 1.5, 1)
        suggested_tp_pips = round(atr_pips * 2.0, 1)
    ...
    return {
        ...
        "atr_pips": round(latest_atr * 10000, 1) if latest_atr else None,
